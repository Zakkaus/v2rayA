      ...
      <b-field :label="$t('tproxyWhiteIpGroups.formName1')">
        <b-select multiple v-model="countryCodes" expanded>
          ...
          <option value="PRIVATE">
            {{ $t("tproxyWhiteIpGroups.private") }}
          </option>
          <option value="US">{{ $t("tproxyWhiteIpGroups.us") }}</option>
          <option value="CLOUDFLARE">
            {{ $t("tproxyWhiteIpGroups.cloudflare") }}
          </option>
        </b-select>
      </b-field>
  ...
  methods: {
    validateCIDRArray(arr) {
      ...
      const invalid = arr
        ...
        .filter(
          (s) => s.length === 0 || !(ipv4Cidr.test(s) || ipv6Cidr.test(s)),
        );
      return invalid.length === 0;
    },
    handleClickSubmit() {
      if (
        !this.validateCIDRArray(
          this.customIps.split("\n").filter((line) => line.trim() !== ""),
        )
      ) {
      ...
      this.$axios({
        ...
        handleResponse(
          ...
          null,
          "tproxyWhiteIpGroups.saveFailed",
        );
      });
    },
  },
